...
回数>20			1.02		選択不可		削除予定
国力0				1			選択不可
トップ			0.7		選択不可
回数%11			0.62		選択可		回数%3と入れ替え ここら辺の倍率でも選択可が欲しいと思って非効率な感じで定義 前期から連続を溜めておけるがそんぐらい良いか
SOL<2万			0.6		選択不可
SOL満				0.6		選択不可
統国%3			0.6		選択不可
回数>9			0.56		選択不可		削除予定
...
国力%12			0.25		選択可
ﾚﾍﾞﾙ%7			0.25		選択可
回数%3			0.22		選択不可		回数%4に変更して回数%11と入れ替え
同盟時			0.212		選択可
=cut
...
#	[0,		'任意',							sub{ return 0.1; },	0.1,	1],
	...
	[17,		'連続回数が 3 の倍数の時',		sub{ return ($m{renzoku_c} > 0 && ($m{renzoku_c} % 2) == 0) ? 0.22 : $timing_base; },	0.3,	0], # 4の倍数にして 11 のやつと入れ替え
	[18,		'連続5回以上の時',				sub{ if ($m{renzoku_c} > 4) { $m{renzoku_c} = 0; return 0.28; } else { return $timing_base; } },	0.3,	1],
	[19,		'連続10回以上の時',				sub{ if ($m{renzoku_c} > 9) { $m{renzoku_c} = 0; return 0.56; } else { return $timing_base; } },	0.3,	0], # 削除予定
	[20,		'連続20回以上の時',				sub{ if ($m{renzoku_c} > 29) { $m{renzoku_c} = 0; return 1.12; } else { return $timing_base; } },	0.3,	0], # 削除予定
	[21,		'連続回数が 11 の倍数の時',	sub{ if ($m{renzoku_c} > 0 && (($m{renzoku_c} % 11) == 0)) { $m{renzoku_c} = 0; return 0.62; } else { return $timing_base; } },	0.3,	1],
	...
	[26,		'ﾚﾍﾞﾙが 7 の倍数の時',			sub{ return $m{lv} % 7 == 0 ? 0.25 : $timing_base; },	0.3,	1],
...
ｽﾃﾀﾞｳﾝ		2								9つあるステの1つが-0～19だし大したことない 当たり
...
孵化値		1.2							孵化値-100だけどﾄﾞﾗｳﾌﾟﾆﾙで孵化値+500引けるし育て屋で預けっぱなしで忘れてたのとかデメリットなしに使える
拘束			1
...
#		my $vv = int((20000 + rand(20000)) * $v);
	...
	[14,	'ｴｸｽｶﾘﾊﾟｰ',				sub{
		...
		$cs{is_die}[$c] = 1;# (int(rand(3)) < 1); # 1/3 の確率でﾉｱ効果
	...
	[16,	'ﾄﾞﾗｳﾌﾟﾆﾙ',				sub{
		...
		# 撃った回数に対して疲労回復出すぎてあんまり美味しい感じしない 疲労減らして孵化値追加
		...
		while (my $player = <$fh>) {
			...
			if ($f == 0) { # 疲労回復
				...
				&regist_you_data($player, 'egg_c',$p{egg_c} + 500);
			}
		}
		close $fh;
