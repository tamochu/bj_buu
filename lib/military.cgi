...
sub form1 {
	...
	if ($config_test) {
		if ($m{tp} == 420) {
			if (int(rand(5)) < 1) {
				$mes .= "u‘Ü•Ô‚µ‚ÌpIv<br>i–¡•û‚Æv‚í‚¹“°X‚Æé“à‚É“ü‚écIj<br>";
				$m{turn} = 200;
			}
			else {
				$mes .= "u“¦~‚ÌpIv<br>i“r’†‚Å“¦‚°‚½U‚è‚ğ‚µ‚Äé“à‚ÉN“ü‚·‚écIj<br>";
				$m{turn} = 100;
			}
		}
	}
...
sub tp_430{ # ’ã@
	...
	if ($config_test) {
		if ($m{turn} > 100 && $m{turn} < 200) {
			$mes .= $m{turn} eq '101' ? "$e2j{food}‚Ìî•ñ‚ğè‚É“ü‚ê‚Ü‚µ‚½!<br>"
...
sub tp_440 { # ’ã@
	...
	if ($config_test) {
		if ($m{turn} > 100 && $m{turn} < 200) {
			$mes .= "$e2j{food}F$cs{food}[$y{country}] <br>"       if $m{turn} >= 101;
			$mes .= "$e2j{money}F$cs{money}[$y{country}] <br>"     if $m{turn} >= 102;
			$mes .= "$e2j{soldier}F$cs{soldier}[$y{country}] <br>" if $m{turn} >= 103;
		}
		elsif ($m{turn} > 200) {
		}
	}
	...
	if ($cmd eq '1') {
		...
		if ($config_test) {
			if ($turn > 100 && $turn < 200) {
				...
				$comment .= '<br>‰ï˜b‚ğ—§‚¿•·‚«‚µ‚Ü‚µ‚½' if $m{turn} > 4;
			}
			elsif ($turn > 200) {
				$comment .= '‰ï˜b‚ğ—§‚¿•·‚«‚µ‚Ü‚µ‚½';
			}
