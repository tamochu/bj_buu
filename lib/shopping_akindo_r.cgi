...
sub tp_100 {
	...
	if ($cmd && -f "$userdir/$shop_id/shop.cgi") {
		my $line = ''; # ”ƒ‚Á‚½ƒAƒCƒeƒ€î•ñ‚ª“ü‚é
		my @lines = (); # ‚»‚Ì‘¼‚Ì¤•i‚ª“ü‚é
		...
		if ($line) {
			...
			if ($m{money} >= $price) {
				...
				my $sell_id = int(rand(1000)+1); # IŠÊŒ ŒÀ‚ÅŒ©‚ê‚éæˆøIDH send_item ‚Å‚Í”š”­–³‹ƒtƒ‰ƒO‚Æ‚µ‚©g‚í‚ê‚Ä‚È‚¢
				...
				# send_item‚ÌƒƒMƒ“ƒO‹@”\‚µ‚Ä‚È‚¢ ‚ ‚Á‚¿‚Ì‚ª‘Š‡“I‚È‚Ì‚Å‚»‚Ì‚¤‚¿ˆÚ“®
...
sub disp_goods {
	...
	# ©•ª‚Ì‚¨“X‚Å”ƒ‚¢•¨‚Å‚«‚Ä‚µ‚Ü‚¤‚ÆA”„ã×İ·İ¸Ş‚ª•ö‰ó‚µ‚Ä‚µ‚Ü‚¤‚Ì‚ÅB
		...
		$mes .= qq|y$m{stock}z$y{name}u$shop_messagev<br>|;
		$mes .= qq|<form method="$method" action="$script_r"><input type="radio" name="file_name" value="0" checked>‚â‚ß‚é<br>|;
		$mes .= qq|<table class="table1"><tr><th>¤•i–¼</th><th>’l’i<br></th></tr>|;
...
sub tp_500 {
	&begin_goods('book', 'html', '–{');
}

sub tp_501 {
	&disp_goods('book', 'html', '–{');
}
