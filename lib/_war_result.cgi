...
#	$v = int($v * 1.05) if $cs{ceo}[$m{country}] eq $m{name};
	...
	# àÂ’m‚Ì’D‘—Í‚ğl—¶‚µí‚ÉàÂ’m‚É‡‚í‚¹‚½ƒJƒEƒ“ƒ^[‚É‚·‚é‚ÆˆÃ•‘¤àÂ’m‚ÅƒSƒŠƒSƒŠí‚ê‚»‚¤ ‚Æ‚è‚ ‚¦‚¸••ˆó‚ÆˆÃ•‚Å’D‘—Í‚ª•Ï‚í‚éd—l‚É‚µ‚Ä‚µ‚Ü‚¤
...
#			$v = $v > 200 ? int(rand(50)+150) : int($v);
			...
			if($m{unit} eq '18'){
				$npc_v = int(rand(750)+750) if $w{world} eq $#world_states && ($m{country} ne $w{country} && $union ne $w{country}) ; # ••ˆó‘¤àÂ’m‚Ì‚İàÂ’m‚Ì’D‘—Í‚É‡‚í‚¹‚½ƒJƒEƒ“ƒ^[—Ê
