...
sub _touitu {
	...
	if ($w{world} eq $#world_states) {
		if ($m{country} eq $w{country} || $union eq $w{country}) { # NPC‘‘¤‚ÌŸ—˜
			...
			&send_twitter("[‚«ˆÅ‚æ‚è–ÚŠo‚ß‚½$cs{name}[$w{country}]‚Ì–ÒÒ’B‚ª$mname‚ğ•M“ª‚Æ‚µ$world_name‘å—¤‚ğx”z‚·‚é");
			$is_npc_win = 1;
		}
		else {
			...
			&send_twitter("$c_m‚Ì$mname‚Æ‚»‚Ì’‡ŠÔ’B‚ª–‚ŠE‚ğÄ‚Ñ••ˆó‚µA$world_name‘å—¤‚É‚Ğ‚Æ‚Æ‚«‚ÌˆÀ‚ç‚¬‚ª‚¨‚Æ‚¸‚ê‚é");
		}
	}
	elsif ($w{world} eq $#world_states-2) {
		...
		&send_twitter("$c_m‚Ì$mname‚ª$world_name‘å—¤‚ğ“ˆê‚·‚é");
		$w{win_countries} = $m{country};
	}
	elsif ($w{world} eq $#world_states-3) {
		...
		if ($union) {
			...
			&send_twitter("$c_m$cs{name}[$union]“¯–¿‚Ì$mname‚ª$world_name‘å—¤‚ğ“ˆê‚·‚é");
		}
		else {
