...
#			if( rand(4) < $npc_war  || ($cs{strong}[$w{country}] < 30000 && rand(3) < $npc_war) ) {
			...
			# (暗黒人数(暗黒の同盟を含む人数) * 16) / 封印人数 = 戦争ｶｳﾝﾀｰの分母 1/n
			# 封印人数の 1/16 の暗黒人数なら 100% 反撃
			# 封印人数の 1/8 の暗黒人数なら 50% 反撃
			# 封印人数の 1/4 の暗黒人数なら 25% 反撃（無改造と同等）例えば 20 vs 80 とか 50 vs 200
			# 封印人数の 1/2 の暗黒人数なら 12.5% 反撃
			# 封印人数の 1/1 の暗黒人数なら 6.25% 反撃
			my @npc_mem = (0, 0);
			$npc_mem[$w{"p_".$_."_".$w{country}}] += $cs{member}[$_] for (1 .. $w{country}-1);
			my $npc_per = ($npc_mem[1] + $cs{member}[$w{country}]) * 16 / $npc_mem[0];
			my $boost_npc_per = $cs{strong}[$w{country}] < 30000 ? 1 : 0;# 国力30000未満 分子+1
			if( rand($npc_per) < (1 + $boost_npc_per) ) { # 確率の加法定理使ってｶｳﾝﾀｰ率計算するの面倒なので人間が分かりやすいように省略
				require './lib/vs_npc.cgi';
				&npc_war;
			}
		}
